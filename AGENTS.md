...
- From the repository root, run `./npc/scripts/run_digital_twin_vivado.sh bitstream --jobs 16 --ip-jobs 4` when a bitstream is required. The script rebuilds `pack-fpga`, refreshes the imported files under `jyd-vivado-proj/`, and runs Vivado through synthesis, implementation, and bitstream generation. Use `impl` when bitstream generation is not needed. Do not use top-level jobs below 16 or IP/OOC jobs below 4 without explicit user approval and the script's `--user-approved-low-jobs` audit flag.
...
Pblock, floorplan, and other placement/routing-only experiments, as well as
synthesis-level directive sweeping/traversal (for example scanning global
retiming, flatten hierarchy, or keep-equivalent-register switches), are
prohibited until the exact candidate's final post-route WNS is greater than
`-0.1 ns`. At `WNS <= -0.1 ns`, only structural RTL/architecture-level
