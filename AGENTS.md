...
Vivado IP simulation models such as `mult_gen_0.sv`, `div_gen_uradix2.sv`, and `blk_mem_gen_2KB.sv` are emitted inline by their Chisel `BlackBox` definitions. Add new IP models to `CHISEL_UNSYNTH_KEYWORDS` in `npc/scripts/chisel.mk`: RTL simulation must compile the emitted model, while synthesis and `pack-fpga` must omit it and bind the module name to the Vivado IP output product instead. Like `blk_mem_gen_dram`, `blk_mem_gen_2KB` supports byte writes; it is a 512 x 32-bit memory with a 9-bit word address, one-cycle synchronous read latency, and read-first read-during-write behavior.
