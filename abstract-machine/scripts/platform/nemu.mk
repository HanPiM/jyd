...
CFLAGS    += -I$(JYD_AM_HOME)/am/src/platform/nemu/include
...
run: insert-arg
	$(MAKE) -C $(JYD_NEMU_HOME) ISA=$(ISA) run ARGS="$(NEMUFLAGS)" IMG=$(IMAGE).bin

gdb: insert-arg
	$(MAKE) -C $(JYD_NEMU_HOME) ISA=$(ISA) gdb ARGS="$(NEMUFLAGS)" IMG=$(IMAGE).bin
