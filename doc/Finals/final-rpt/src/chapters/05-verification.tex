...
src0 完成 37 条功能测试并进入 halt。既有日志保留周期数与动态指令数作为回归过程记录；\textbf{IPC、
CPI 与按目标频率换算的时间将在取得新的同口径仿真结果后统一填写。}
...
src1 既有日志保留周期数与动态指令数作为回归过程记录；IPC、CPI 与时间等待新的同口径仿真结果。
Vivado Simulation 中数码管写入 37000000，表明 37 条功能测试完成；代表性波形统一见上图。
...
src2 既有日志保留周期数与动态指令数作为回归过程记录；IPC、CPI 与时间等待新的同口径仿真结果。
其 Vivado Simulation 同样完成数码管结果写入，波形判读口径与 src0 一致。
...
src0&1988M&1417M&\MetricPending{新仿真}&\MetricPending{新仿真}&\MetricPending{新仿真}\\
src1&1973M&1304M&\MetricPending{新仿真}&\MetricPending{新仿真}&\MetricPending{新仿真}\\
src2&2585M&1849M&\MetricPending{新仿真}&\MetricPending{新仿真}&\MetricPending{新仿真}\\
\bottomrule\end{tabularx}
\end{table}

\subsubsection{CoreMark 与 Dhrystone}

\PlaceholderFigure{CoreMark 新仿真结果}{待补充迭代次数、周期数、指令数、IPC、CPI、目标频率、换算时间、CoreMark 得分与 CRC。}

CoreMark 的内部统计项暂以占位符保留：周期数 \MetricPending{新仿真}，指令数
\MetricPending{新仿真}，IPC \MetricPending{新仿真}，CPI \MetricPending{新仿真}，运行时间
\MetricPending{新仿真}，CoreMark 得分 \MetricPending{新仿真}。\textbf{最终结果须同时给出迭代次数、
目标频率和四项 CRC，确保性能值与正确性证据同口径。}

\PlaceholderFigure{Dhrystone 新仿真结果}{待补充迭代次数、周期数、指令数、IPC、CPI、目标频率、换算时间、Dhrystones/s 与 DMIPS。}

Dhrystone 的内部统计项暂以占位符保留：周期数 \MetricPending{新仿真}，指令数
\MetricPending{新仿真}，IPC \MetricPending{新仿真}，CPI \MetricPending{新仿真}，运行时间
\MetricPending{新仿真}，Dhrystones/s 与 DMIPS \MetricPending{新仿真}。新结果将统一注明迭代次数与
目标频率。
...
正式负载多次板测&完成&当前编译路径三次相同 6.691154040\,s，四项 CRC 一致\\
