...
\coordinate (outleft) at ($(out.west)+(-0.34,0)$);
\coordinate (reqrightlow) at ($(req.east |- out.south)+(0.28,-0.52)$);
\coordinate (idlebelow) at ($(idle.south)+(0.20,-0.55)$);
\coordinate (reqrighthigh) at (reqrightlow |- idlebelow);
\draw[a,preaction={draw=white,line width=4.0pt}]
  (out.west) -- (outleft) |- node[pos=0.34,below]{结果被消费}(reqrightlow)
