...
本项目在初赛 RV32I 五级流水 CPU 的基础上继续扩展，当前支持赛题所需的 37 条 RV32I 整数、
访存与控制流指令，并扩展支持 RV32M、Zicsr 六条 CSR 读改写指令，以及
\texttt{ECALL}/\texttt{MRET} 机器态控制流。
CPU 保持 IFU--IDU--EXU--LSU--WBU 五级顺序单发射架构，并新增多周期乘除法、2 KiB 直映射
D-cache、基于 FPGA 分布式 RAM 的双副本寄存器堆，以及 PC、BTB 标记与跳转目标位宽削减等优化。

设计在 Vivado 2024.2 下完成综合与实现，核心时钟频率达到
\textcolor{red}{\textbf{280 MHz}}，最差时序裕量 WNS 为 \textcolor{red}{\textbf{+0.050 ns}}，无建立时间违例。
初赛程序 \texttt{src0}、\texttt{src1}、\texttt{src2} 的运行时间分别为
\textbf{7.101 s、7.047 s、9.233 s}；新程序 \texttt{withoutMext} 与 \texttt{withMext}
分别用时 \textbf{3.890 s} 与 \textbf{2.020 s}。

\begin{figure}[H]
\centering
\includegraphics[width=0.96\linewidth]{快速预览-1.png}\\[0.5em]
\includegraphics[width=0.96\linewidth]{快速预览-slack.png}
\caption{分赛区决赛版本快速预览与时序结果}
\end{figure}
...
\begin{figure}[H]
  \centering
	\includegraphics[width=\textwidth,trim={15pt 130pt 15pt 45pt},clip]{cpu-arch.pdf}
	\caption{架构图}
\end{figure}
...
本项目在初赛完整 CPU、SoC 与板级适配成果上完成了分赛区决赛版本迭代。功能方面，处理器当前支持
赛题所需的 37 条 RV32I 整数、访存与控制流指令，并扩展支持 RV32M、Zicsr 六条 CSR 读改写指令，
以及 \texttt{ECALL}/\texttt{MRET} 机器态控制流。这些功能为更完整的机器态软件适配提供基础，
但完整异常、中断体系和 RT-Thread 适配仍有待后续完善。
乘除法严格按 RISC-V 语义处理有符号、高位结果、除零和溢出边界，并通过多周期握手接入流水线，
没有破坏顺序提交和 RAW 相关控制。

微结构方面，CPU 继续采用 IFU--IDU--EXU--LSU--WBU 五级顺序单发射流水。IFU 保留初赛已验证的
四状态连续取指机制，可在响应消费的同周期接收下一 PC 并发出新请求；IDU 负责译码、预计算、
EXU/LSU/WBU 三级旁路和结果未就绪停顿；EXU 统一完成单周期 ALU、多周期乘除法、访存请求构造和
控制流裁决；LSU 独立承接存储响应与缓存旁路；WBU 完成 load 数据扩展以及 GPR/CSR 最终提交。
分级职责、接口表和 ready/valid 反压共同保证了连续流动、停顿与冲刷场景下的数据一致性。

控制流方面，16 项直接映射 BTB 配合 BTFN 静态策略预测 JAL 和后向条件分支，预测记录随指令进入
流水线并由 EXU 以真实结果校验。预测错误、JALR、异常进入与返回均通过统一重定向路径冲刷错误指令。
在容量由 8 项提升到 16 项的同时，将 PC、tag 和 target 缩减为 \texttt{[16:2]} 的 15 位字地址，
避免扩大预测比较与目标选择关键路径。

存储系统方面，IROM 继续使用一周期 BRAM 并由 IFU 隐藏读延迟；DRAM 为改善时序改为两周期响应。
2 KiB 直映射 D-cache 对 DRAM 对齐 \texttt{LW} 提供 LSU 命中旁路，miss 由 WBU 回填，store 触发
对应行失效，store epoch 阻止较老 load 的迟到响应覆盖年轻 store 的失效结果。GPR 使用两份 32$\times$32 分布式 RAM 镜像实现
双异步读、单同步写，在保持体系结构语义的同时降低 LUT Mux 和布线压力。JYDSoC、JYDFPGATop、
外设地址映射、SimpleBus 数据接口及可选 AXI4 桥接仍保持完整，CPU 内核可在仿真、数字孪生和
通用 SoC 环境间复用。

实现结果表明，决赛版本在 Vivado 2024.2 下达到 \textbf{280 MHz}，最差时序裕量 WNS 为
\textbf{+0.050 ns}，无建立时间违例。\texttt{src0/src1/src2} 分别用时
\textbf{7.101/7.047/9.233 s}，相对初赛分别降低 18.4\%、28.1\% 和 12.8\%，三组平均降低
约 19.7\%。M 扩展新测评中，硬件 M 扩展将运行时间由 3.890 s 降至 2.020 s，获得约
\textbf{1.93 倍}加速。由此可见，本阶段并非对初赛架构的简单替换，而是在保留已验证完整性的基础上，
围绕指令能力、存储延迟、控制流、资源映射和物理时序进行协同优化，形成了功能、频率与有效吞吐率
更均衡的 FPGA RISC-V CPU 实现。
