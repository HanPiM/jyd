...
Within this repository, the official source is built as an Abstract Machine
program. The source files and the AM port are all under `src/`; the Makefile
reuses the AM startup, device, timer, linker, and `putch()` implementation.
...
configuration-specific subdirectory, so changing a compile-time option cannot
reuse an object or archive from another configuration. These are Make variables,
for example:
...
`0x80200050`. FPGA hardware increments it at 50 MHz, so reporting uses
`COREMARK_TICKS_PER_SEC=50000000`; elapsed seconds and iterations per second are
computed in software. The RT-Thread embedding continues to use its millisecond
AM timer path.
...
floating-point ABI is required. EEMBC's `barebones/ee_printf.c` and `cvt.c`
provide formatting, while the compiler soft-float ABI calls and `modf`
truncation are backed by the Berkeley SoftFloat library under
`abstract-machine/softfloat`. That AM library is fetched once and cached by
target ISA configuration rather than by CoreMark iteration/data settings. The
