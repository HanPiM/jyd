...
scratch=$(mktemp -d "${TMPDIR:-/tmp}/xbmul-md-check.XXXXXX")
trap 'rm -rf -- "$scratch"' EXIT HUP INT TERM

common_flags='-O3 -march=rv32im_zbc_zicsr -mabi=ilp32'
...
"$cc" $core_flags $core_includes '-DARCH_H="arch/riscv.h"' -S "$coremark_dir/src/core_util.c" -o "$scratch/crc-control.s"
"$cc" $core_flags $core_includes '-DARCH_H="arch/riscv.h"' -mxcrcu8 -S "$coremark_dir/src/core_util.c" -o "$scratch/core-xcrcu8.s"
...
if grep -Fq '.insn r 0x0b, 0, 0' "$scratch/crc-control.s"; then
    echo "CRC control unexpectedly contains xcrcu8" >&2
    exit 1
fi
crc_matches=$(grep -Fc '.insn r 0x0b, 0, 0' "$scratch/core-xcrcu8.s")
test "$crc_matches" -gt 0
...
echo "unmodified core_util.c CRC matches: $crc_matches"
