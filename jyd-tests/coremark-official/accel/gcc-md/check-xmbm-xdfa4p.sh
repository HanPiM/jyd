...
scratch=$(mktemp -d "${TMPDIR:-/tmp}/xmbm-xdfa4p-md-check.XXXXXX")
...
sed -e 's/core_bench_state/audit_state_scan/g' \
    ...
    "$coremark_dir/src/core_state.c" > "$scratch/state-renamed.c"
"$cc" $common_flags $includes '-DARCH_H="arch/riscv.h"' -mxdfa4p \
    -fdump-tree-clippedscore -S "$scratch/state-renamed.c" \
    -o "$scratch/state-renamed.s"
grep -Fq 'recognized numeric-token DFA scan' "$scratch"/state-renamed.c.*.clippedscore
test "$(grep -Fc '.insn r 0x5b, 5, 2' "$scratch/state-renamed.s")" -eq 2

echo "unmodified core_matrix.c xmbm sites: $xmbm_matches"
echo "unmodified core_state.c xdfa4p step sites: $step_matches"
echo "renamed DFA source shape: PASS"
