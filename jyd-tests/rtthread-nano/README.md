...
Building the patched GCC toolchain from source requires the normal
GMP/MPFR/MPC development dependencies. Embedded CoreMark objects use the same GCC MD defaults as `coremark-official`;
...
make -C jyd-tests/rtthread-nano ARCH=riscv32-jyd \
  CROSS_COMPILE=/path/to/md-gcc/bin/riscv64-unknown-linux-gnu- run
```

The imported options affect only the embedded benchmark; RT-Thread objects use
the same base-ISA flags in every build.
