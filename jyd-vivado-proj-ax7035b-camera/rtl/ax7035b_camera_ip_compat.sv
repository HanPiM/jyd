...
// Camera demo memory budget: 16 KiB distributed IROM and 48 KiB distributed
// DRAM. Their external names and read latencies exactly match JYDFPGATop.
module blk_mem_gen_irom (
    input wire clka, input wire ena, input wire [17:0] addra,
    output reg [31:0] douta
);
    (* rom_style = "distributed" *) reg [31:0] storage [0:4095];
    integer i;
    initial begin
        for (i=0; i<4096; i=i+1) storage[i]=0;
        $readmemh("irom.mem", storage);
    end
    always @(posedge clka) if (ena) douta <= storage[addra[11:0]];
    wire unused_addra = |addra[17:12];
endmodule
...
module blk_mem_gen_dram (
    ...
    (* ram_style = "distributed" *) reg [31:0] storage [0:12287];
    reg [31:0] memory_output;
    integer i;
    initial begin
        for (i=0; i<12288; i=i+1) storage[i]=0;
