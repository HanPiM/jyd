...
class JYDAHT10FPGABlackBox extends BlackBox with HasBlackBoxResource {
  ...
  val io = IO(new Bundle {
    ...
    val local_data_valid = Output(Bool())
  })
...
class SimpleBusFPGAAHT10 extends Module {
  val io = IO(new Bundle {
    ...
    val localTemperatureX10 = Output(SInt(16.W))
    val localHumidityX10 = Output(UInt(16.W))
    val localDataValid = Output(Bool())
  })
  io.bus.dontCareResp()
  ...
  io.localTemperatureX10 := sensor.io.local_temperature_x10
  io.localHumidityX10    := sensor.io.local_humidity_x10
  io.localDataValid      := sensor.io.local_data_valid
