...
flowchart TD
    ...
    D -->|通过| E[代表性软件样本仿真]
    E -->|无收益或数据不支持假设| R
    E -->|有足够预期收益| F[同步 Vivado IP 与 RTL 配置]
    F --> G[Vivado 综合与实现]
    G -->|大时序违例| R
    G -->|小时序违例| H[尝试有限个实现策略]
    H -->|均不满足| R
...
如果该样本与真实上板负载差异明显，应直接运行固定主样例的 RTL perf，并以主样例数据决定是否继续。
...
## 4. 用代表性仿真做 Vivado 前筛选

Vivado 成本较高，先用 RTL 仿真回答两个问题：

1. 优化是否真的减少目标负载的周期？
2. 收益是否大到值得承担综合、实现和上板成本？

固定主样例可用轻量 perf 模式运行，示例：
...
性能计数必须与上板时间能互相解释。若仿真周期按目标频率换算后与上板明显不一致，先检查输入镜像、频率、仿真模型和 IP 延迟是否一致。
